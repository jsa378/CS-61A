...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    is_cur_mobile_balanced = total_mass(end(left(m))) * length(left(m)) == total_mass(end(right(m))) * length(right(m))
    if is_planet(end(left(m))) and is_planet(end(right(m))):
        return is_cur_mobile_balanced
    else:
        if not is_planet(end(left(m))) and is_planet(end(right(m))):
            return balanced(end(left(m))) and is_cur_mobile_balanced
        elif not is_planet(end(right(m))) and is_planet(end(left(m))):
            return balanced(end(right(m))) and is_cur_mobile_balanced
        else:
            return balanced(end(left(m))) and balanced(end(right(m))) and is_cur_mobile_balanced
